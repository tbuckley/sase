...
Handler.prototype.generateSequences = function(terminatingInstance) {
  ...
  terminatingInstance.dfs(null, function(inst) {
    if(inst.position === 0) {
      // Base case: create a sequence of length 1 in stack 0
      stacks[0].push([inst.value]);
    } else {
      // Inductive step: add symbol to the sequences of length n-1, and add these to stack n-1
      stacks[inst.position - 1].forEach(function(seq) {
        seq.push(inst.value);
      });
      stacks[inst.position] = stacks[inst.position].concat(stacks[inst.position - 1]);
      stacks[inst.position - 1] = [];
    }
  });
